...
// LLM에 필요한 필드만 남긴다 — 입력 토큰이 줄면 지연도 준다. 순서는 배열 순서가 전달한다.
...
## 실행 요약 (Digest)
각 스팬은 실제 실행 기록의 한 구간이며, 배열은 실행 순서 그대로 나열되어 있습니다. 당신은 이 spanId만 참조할 수 있습니다.
iterations는 그 반복 구간이 실제로 돈 총 횟수입니다 — 반복 횟수를 언급할 땐 이 숫자만 사용하세요 (재생기가 자동으로 "(총 N회 반복)"을 덧붙이므로 narration에 횟수를 직접 쓰지 않아도 됩니다).
${JSON.stringify(promptSpans(digest))}
${digest.aliasNote ? `\n참고 — 별칭 관계: ${digest.aliasNote}` : ''}

## 출력 형식 (JSON만, 다른 텍스트 금지)
{"chapters":[{"title":"챕터 제목","scenes":[{"spanRef":"스팬ID","primitive":"variables|callStack|sequence|objectGraph|generic","focus":["변수명"],"pacing":"slow|normal|fast","direction":["zoom|hold|skip"],"narration":{"template":"관찰형 한 문장, 값은 {키}로","bindings":{"키":{"name":"변수명"}}}}]}],"staging":{"grid":["변수명"],"noGrid":[]}}
...
11. staging(선택, 최상위): 자료의 표현 선택. 미로·보드·DP 테이블처럼 **공간(2차원)으로 이해해야 하는 자료**의 변수명을 "grid"에 — 같은 길이 문자열 행 리스트(["S.#", "..#"])도 격자가 될 수 있다. 반대로 2차원 리스트지만 표가 아닌 자료(우연히 모양만 같은 데이터)는 "noGrid"에. 격자 자격 검사와 그리기는 시스템이 하므로 이름만 적을 것. 확신 없으면 생략.`
