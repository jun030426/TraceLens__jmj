...
| 08-19 | **토큰은 슬롯 층 위의 독립 층** — 토큰이 각자의 칸 그룹 안에 살면 SVG 문서 순서가 곧 겹침 순서라, 오른쪽으로 나는 토큰이 **높은 인덱스 칸의 불투명 배경 뒤로 들어간다** (실측: 스왑 33개 순간, 최대 636px² 가림. 왼쪽으로 나는 토큰과 시프트는 나중에 그려져 멀쩡하므로 한쪽만 사라지는 비대칭이 더 어색했다). 렌더를 **슬롯 층(배경·완성칠·플래시·링·번호) 다음에 토큰 층(막대+글자)** 두 패스로 분리한다 — 토큰은 좌표가 이미 상자 로컬 절대값이라 위치는 그대로고, 모든 슬롯 위를 지나간다. 대신 칸의 등장·퇴장·스윕·커튼콜 펄스는 슬롯과 토큰을 **함께** 대상으로 잡아야 한다 (한쪽만 잡으면 자리와 물건이 따로 논다). 런타임 DOM 재정렬은 기각 — 스크럽에서 역방향 복원이 보장되지 않는다 | "슬롯은 자리, 토큰은 물건"이라는 문장의 정직한 구현은 토큰 층이 실제로 슬롯 층 위에 뜨는 것이다. z-index 없는 SVG에서 겹침 순서는 설계이지 우연이 아니다 |
