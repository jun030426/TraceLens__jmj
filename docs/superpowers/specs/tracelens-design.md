...
**남은 것**:
- **파일럿 실행** — `/app` 무대 녹화 영상 + 설문지 방식 (전용 /demo 화면은 08-19 삭제 — 참가자별 재생 편차가 없어 통제가 오히려 좋다). **소재는 재선정 중**: 구 6편은 끈·고정 무대 시절 설계라 폐기, 새 세트는 은유 부품 커버 기준으로 선정 (pilot-guide)
...
| 08-19 | **/demo 삭제, 파일럿은 녹화 영상 + 설문지로** — 전용 화면·픽스처·생성 스크립트 제거. **구 소재 6편도 폐기** (끈 시대 설계 — 현 은유 부품이 한 편에도 등장하지 않음), 새 세트는 선정 기준(은유 커버·개념 커버·층화)으로 재선정 | 네비 미연결 화면의 유지비 제거 + 녹화가 재생 편차를 없애 통제에 유리 + 소재는 지금 제품을 검증해야 한다 |
