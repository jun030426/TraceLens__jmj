...
- easing 현행: 등장 back.out, 이동 power2.inOut, 스왑은 두 토큰의 **레인 교차**(몸을 낮춰 위·아래 ±11 레인으로 비껴간 뒤 back settle로 펴짐 — 상자 안에 머문다). 타임라인 라벨은 절대 위치 — 샷 경계와 초 단위로 일치 (자막-화면 드리프트 금지).
...
| 08-19 | **스왑 = 레인 교차 (포물선 폐기)** — 첫 안의 상하 포물선(−30/+18)은 칸 높이(48)를 넘겨 **토큰이 상자 밖으로 나갔다** (실측 스크린샷: 위 토큰은 이름표 띠·저울 자리까지 올라가고 아래 토큰은 하단 테두리에 걸쳐 렌더 버그로 읽힘). 상하 여유는 구조적으로 없다 — 위는 이름표 띠와 착지한 저울, 아래는 칸 번호가 점유한다. 그래서 진폭을 키우는 대신 **상자 안에서 비껴가게** 바꾼다: 두 토큰이 0.66으로 몸을 낮춰 ±11 레인으로 갈라져 지나간 뒤 back settle로 펴진다. 수용 기준은 눈이 아니라 기하 — **비행 전 구간에서 토큰 사각형 ⊆ 상자 사각형**을 브라우저 실측으로 증명한다. 시프트(직선·원래 크기)와 스왑(레인·축소)의 서명 구분은 유지 | 은유 무대는 부품끼리 자리를 다툰다 — 저울을 칸 위로 내린 결정이 위쪽 공간을 이미 썼으므로, 물성 연출은 남은 공간 안에서 성립해야 한다. 테두리를 가르는 애니메이션은 물성이 아니라 고장으로 읽힌다 |
